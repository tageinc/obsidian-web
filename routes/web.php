...
Route::get('/delta', [Delta::class, 'showDelta'])->name('delta');
    ...
    Route::get('/developer-workspace', [DeveloperWorkspaceController::class, 'index'])->middleware('browser.developer')->name('developer-workspace');
    Route::middleware('browser.developer')->prefix('developer-workspace/api-keys')->name('developer.api-keys.')->group(function () {
        Route::get('/', [\App\Http\Controllers\ExternalApiKeyController::class, 'index'])->name('index');
        Route::post('/', [\App\Http\Controllers\ExternalApiKeyController::class, 'store'])->middleware('throttle:20,1')->name('store');
        Route::post('/{key}/revoke', [\App\Http\Controllers\ExternalApiKeyController::class, 'revoke'])->name('revoke');
    });
    Route::get('/admin-control-center', [DeveloperWorkspaceController::class, 'legacyRedirect'])->middleware('browser.developer')->name('admin-control-center');
    Route::post('/upload-firmware', [DeveloperWorkspaceController::class, 'uploadFirmware'])->middleware('browser.developer')->name('uploadFirmware');
    Route::post('/upload-config', [DeveloperWorkspaceController::class, 'uploadConfig'])->middleware('browser.developer')->name('uploadConfig');
    Route::get('/firmware-file/version/{version?}', [DeveloperWorkspaceController::class, 'serveFirmwareByVersion']);
    Route::get('/config-file/version/{version?}', [DeveloperWorkspaceController::class, 'serveConfigByVersion']);
    Route::get('/firmware-file/prefix/{prefix?}', [DeveloperWorkspaceController::class, 'serveFirmwareByPrefix']);
    Route::get('/config-file/prefix/{prefix?}', [DeveloperWorkspaceController::class, 'serveConfigByPrefix']);
    Route::get('/firmware-version/{prefix?}', [DeveloperWorkspaceController::class, 'getLatestFirmwareVersionNumber']);
    Route::get('/config-version/{prefix?}', [DeveloperWorkspaceController::class, 'getLatestConfigVersionNumber']);
    Route::get('/devices/{id}', [ViewDeviceController::class, 'show'])->middleware('browser.device')->name('devices.show');
    Route::get('/devices/{id}/report', [DeviceHistoryReportController::class, '__invoke'])->middleware('browser.device')->name('devices.report');
    Route::get('/devices/{id}/telemetry', [DeviceTelemetryController::class, '__invoke'])->middleware('browser.device')->name('devices.telemetry');
