...
class DeviceInlineEditTest extends TestCase
{
    ...
    public function test_inline_updates_preserve_history_and_reject_protected_fields(): void
    {
        ...
        $this->actingAs($owner)->patchJson('/devices/'.$device->id, ['serial_no' => 'renamed', 'sku' => 'SP2', 'order_no' => 'ORDER'])
            ->assertOk()->assertJsonPath('values.serial_no', 'renamed');
        foreach (['solar_tracker_logs', 'solar_tracker_remote_controls', 'geocode'] as $table) {
            $this->assertDatabaseHas($table, ['serial_no' => 'renamed']);
            $this->assertDatabaseMissing($table, ['serial_no' => 'original']);
        }
        $this->patchJson('/devices/'.$device->id, ['latitude' => 34.1, 'longitude' => -118.2])->assertOk();
        $this->assertDatabaseHas('geocode', ['serial_no' => 'renamed', 'status' => 'sleep', 'latitude' => 34.1]);
        $this->patchJson('/devices/'.$device->id, ['state' => 'archived', 'status' => 'online'])->assertUnprocessable();
    ...
    public function test_modal_updates_identity_and_preserves_linked_history(): void
    {
        ...
        $fields = ['name' => 'Tracker', 'address_1' => '123 Main', 'address_2' => null,
            ...
            'serial_no' => 'after', 'sku' => 'SP2', 'order_no' => 'ORDER-2'];
        $this->actingAs($owner)->putJson('/edit-device/'.$device->id, $fields, ['X-Obsidian-Modal' => '1'])->assertOk();
        $this->assertDatabaseHas('devices', ['id' => $device->id, 'serial_no' => 'after', 'sku' => 'SP2', 'order_no' => 'ORDER-2']);
        $this->assertDatabaseHas('solar_tracker_logs', ['serial_no' => 'after', 'ps1' => 42]);
        $this->assertDatabaseMissing('solar_tracker_logs', ['serial_no' => 'before']);
        config(['frontend.vue3.device_edit' => true]);
        $this->getJson('/edit-device/'.$device->id, ['X-Obsidian-Modal' => '1'])
            ->assertOk()->assertJsonPath('props.values.serial_no', 'after')->assertJsonPath('props.values.sku', 'SP2')->assertJsonPath('props.values.order_no', 'ORDER-2');
        DB::table('solar_tracker_logs')->insert(['serial_no' => 'taken', 'ps1' => 10]);
        $this->putJson('/edit-device/'.$device->id, array_merge($fields, ['serial_no' => 'taken']))->assertUnprocessable();
        $this->assertSame('after', $device->fresh()->serial_no);
    }

    public function test_serial_collision_is_rejected_and_edit_page_redirects_to_modal(): void
    {
        $owner = User::factory()->create();
        $device = Device::factory()->create(['user_id' => $owner->id, 'serial_no' => 'original']);
        DB::table('solar_tracker_logs')->insert(['serial_no' => 'occupied', 'ps1' => 12]);
        $this->actingAs($owner)->patchJson('/devices/'.$device->id, ['serial_no' => 'occupied'])->assertUnprocessable();
